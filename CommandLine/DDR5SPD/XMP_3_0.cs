...
namespace DDR5XMPEditor.DDR5SPD
{
    public class XMP_3_0 : PropertyChangedBase
    {
        ...
        public unsafe struct RawXMPProfile
        {  /* Profile1(704-767),  Profile2(768-831),  Profile3(832-895),  Profile4(896-959),  Profile5(960-1023) */
            public byte vpp;        // Byte 704 Module VPP Voltage Level
            public byte vdd;        // Byte 705 Module VDD Voltage Level      
            public byte vddq;       // Byte 706 Module VDDQ Voltage Level
            public byte unkown_03;
            public byte vmemctrl;   // Byte 708 Memory Controller Voltage Level
            public fixed byte minCycleTime[2];        // Byte 709-710 Minimum Cycle Time (tCKAVGmin)
            public fixed byte clSupported[5];        // Byte 711-715 CAS Latencies Supported
            public byte unkown_0A;  
            public fixed byte tAA[2];        // Byte 717-718  Minimum CAS Latency Time (tAAmin)   
            public fixed byte tRCD[2];       // Byte 719-720  Minimum RAS# to CAS# Delay Time (tRCDmin)
            public fixed byte tRP[2];        // Byte 721-722  Minimum Row Precharge Time (tRPmin)  
            public fixed byte tRAS[2];       // Byte 723-724  Minimum Active to Precharge Time(tRASmin),
            public fixed byte tRC[2];        // Byte 725-726  Minimum Active to Active/Refresh Time (tRCmin)  
            public fixed byte tWR[2];        // Byte 727-728  Minimum Write Recovery Time  
            public fixed byte tRFC1[2];        // Byte 729-730  Minimum Refresh Recovery Delay tRFC1
            public fixed byte tRFC2[2];        // Byte 731-732  Minimum Refresh Recovery Delay tRFC2
            public fixed byte tRFC[2];         // Byte 733-734  Minimum Refresh Recovery Delay (Same Bank) tRFCsb
                                               // RSVD for Enthusiast / Certified Settings Profile 1
            public fixed byte tRRD_L[2];       // Byte 735-736 
            public byte tRRD_L_lowerLimit;     // Byte 737
            public fixed byte tCCD_L_WR[2];    // Byte 738-739
            public byte tCCD_L_WR_lowerLimit;  // Byte 740
            public fixed byte tCCD_L_WR2[2];   // Byte 741-742
            public byte tCCD_L_WR2_lowerLimit; // Byte 743
            public fixed byte tCCD_L_WTR[2];   // Byte 744-745
            public byte tCCD_L_WTR_lowerLimit; // Byte 746
            public fixed byte tCCD_S_WTR[2];   // Byte 747-748
            public byte tCCD_S_WTR_lowerLimit; // Byte 749
            public fixed byte tCCD_L[2];       // Byte 750-751
            public byte tCCD_L_lowerLimit;     // Byte 752
            public fixed byte tRTP[2];         // Byte 753-754
            public byte tRTP_lowerLimit;       // Byte 755
            public fixed byte tFAW[2];         // Byte 756-757
            public byte tFAW_lowerLimit;       // Byte 758
            public byte unk_07;       // Byte 759
            public byte unk_08;       // Byte 760
            public byte unk_09;       // Byte 761
            public byte unk_0A;       // Byte 762
            public byte memory_boost_realtime_training;    // Byte 763
            public byte commandRate;        // Byte 764  System Command Rate Mode
            public byte unk_0D;        // Byte 765

            // Byte 0x3E-0x4F
            public fixed byte checksum[2];        // Byte 766-767  Cyclical Redundancy Code (CRC) for Profile
        }
        ...
        public static readonly CommandRatesEnum[] commandRatesMap = new CommandRatesEnum[(int)CommandRatesEnum.Count]
        {
        ...
        public RawXMPProfile rawXMPProfile;
        ushort profileNo;
        ...
        public unsafe double? Frequency
        {
            get
            {
                ushort frq = Utilities.ConvertBytes(rawXMPProfile.minCycleTime[0], rawXMPProfile.minCycleTime[1]);
